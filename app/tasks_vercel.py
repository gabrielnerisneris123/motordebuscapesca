"""
Tasks adaptadas para Vercel (serverless).
Executam de forma síncrona já que não há Celery no serverless.
"""

import asyncio
from loguru import logger
from app.celery_app_vercel import celery_app
...
def task_ciclo_descoberta(self):
    ...
    from app.database_vercel import AsyncSessionLocal
    ...
    async def _executar():
        async with AsyncSessionLocal() as db:
            result = await db.execute(
                ...
                .limit(5)  # Limitado no serverless
            )
    ...
    async def _executar():
        async with AsyncSessionLocal() as db:
            result = await db.execute(
                ...
                .limit(50)  # Limitado no serverless
            )
...
def task_coletar_fonte(self, fonte_id: int):
    ...
    try:
        ...
        logger.error(f"Erro ao coletar fonte {fonte_id}: {e}")
        return {"erro": str(e)}
